...
```powershell
# Java: clase#método
docker run --rm -v "${PWD}:/workspace" -w /workspace maven:3.9.16-eclipse-temurin-26 mvn -B -pl services/core-api -am "-Dtest=DownloadJobServiceTest#createsPartialAnonymousJobWithOmittedItem" "-Dsurefire.failIfNoSpecifiedTests=false" test
...
| `POST` | `/api/v1/auth/magic-link/request` | Público | Solicita un enlace de acceso por correo. |
| `POST` | `/api/v1/auth/magic-link/confirm`, `/api/v1/auth/logout` | Público | Consume el enlace o cierra la sesión de usuario. |
