...
- `searchMode` solo admite `lexical` y `semantic`; omitirlo en la API equivale a `lexical`. El modo semántico usa la colección producida por embeddings como conjunto de resultados, pero MySQL aplica como criterio principal el orden solicitado (`downloads|updated|name`) y usa la relevancia como desempate. Degrada la petición completa a literal ante timeout, 401, 5xx, índice incompleto o más de 20.000 candidatos. Ese techo debe mantenerse por encima del catálogo público para que MySQL pueda aplicar filtros, facetas y paginación sobre una enumeración semántica completa.
